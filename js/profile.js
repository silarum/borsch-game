...
function renderAdminPanel() {
    ...
    let tabsHtml = '';
    tabs.forEach(t => {
        tabsHtml += `<button class="admin-tab ${currentAdminTab === t.id ? 'active' : ''}" onclick="switchAdminTab('${t.id}')">${t.icon} ${t.label}</button>`;
    });
    content.innerHTML = `
        <div style="background:#0d0d1a; border:2px solid #e74c3c; border-radius:20px; width:95%; max-width:500px; height:90vh; display:flex; flex-direction:column; overflow:hidden;">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:4px; padding:10px; border-bottom:1px solid rgba(255,255,255,0.1);">${tabsHtml}</div>
    ...
    if (!document.getElementById('admin-styles')) {
        ...
        style.textContent = `
            .admin-tab { padding:8px 12px; border:none; border-radius:8px; font-size:0.75rem; font-weight:bold; cursor:pointer; background:rgba(255,255,255,0.05); color:#aaa; transition:all 0.2s; white-space:nowrap; }
...
window.switchAdminTab = function(tabId) { currentAdminTab = tabId; renderAdminPanel(); };
...
async function loadDashboard(container) {
    try {
        ...
        container.innerHTML = `
            ...
            <button class="admin-btn admin-btn-warning" onclick="switchAdminTab('balance')" style="width:100%; margin:4px 0;">💰 Управление балансом</button>
            <button class="admin-btn admin-btn-success" onclick="switchAdminTab('withdrawals')" style="width:100%; margin:4px 0;">📤 Заявки на вывод</button>
    ...
    try {
        ...
        players.forEach(p => {
            html += `<tr><td>${p.id}</td><td>${p.nickname||'-'}</td><td>${(p.rum||0).toLocaleString()}</td><td>${parseFloat(p.srum||0).toFixed(2)}</td><td>${parseFloat(p.usdt||0).toFixed(2)}</td><td>${parseFloat(p.ton||0).toFixed(2)}</td>
                <td><button class="admin-btn admin-btn-info" onclick="editPlayer(${p.id})" style="padding:4px 8px; font-size:0.65rem;">✏️</button>
                <button class="admin-btn admin-btn-danger" onclick="deletePlayer(${p.id})" style="padding:4px 8px; font-size:0.65rem;">🗑️</button></td></tr>`;
        });
        html += '</tbody></table>';
...
window.editPlayer = function(id) {
    ...
    }).then(() => { alert('Обновлён'); loadAdminTabContent('players'); });
};
...
window.deletePlayer = function(id) {
    ...
    fetch(`${SUPABASE_URL}/rest/v1/users?id=eq.${id}`, { method: 'DELETE', headers: { 'apikey': SUPABASE_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_ANON_KEY}` } })
        .then(() => { alert('Удалён'); loadAdminTabContent('players'); });
};

// ================== УПРАВЛЕНИЕ БАЛАНСОМ ==================
function loadBalanceManager(container) {
    container.innerHTML = `
        <h4 style="color:#FFD700; margin-bottom:10px;">💰 Управление балансом игрока</h4>
        <input type="text" id="bal-nickname" placeholder="Никнейм игрока" class="admin-input">
        <p style="color:#aaa; font-size:0.7rem; margin:5px 0;">Выберите валюту и действие:</p>
        <select id="bal-currency" class="admin-input">
            <option value="rum">🪙 RUM</option>
            <option value="srum">💎 SRUM</option>
            <option value="usdt">💵 USDT</option>
            <option value="ton">⚡ TON</option>
        </select>
        <select id="bal-action" class="admin-input">
            <option value="set">📌 Установить точное значение</option>
            <option value="add">➕ Начислить (добавить)</option>
            <option value="subtract">➖ Списать (вычесть)</option>
        </select>
        <input type="number" id="bal-amount" placeholder="Сумма" step="0.01" class="admin-input">
        <button class="admin-btn admin-btn-warning" onclick="updateBalance()" style="width:100%; margin-top:10px;">✅ Применить</button>
...
window.updateBalance = async function() {
    ...
    if (isNaN(amount) || amount < 0) { resultP.textContent = 'Введите корректную сумму'; return; }
    ...
    try {
        ...
        switch (action) {
            ...
            case 'add': newValue = current + amount; break;
            case 'subtract': newValue = Math.max(0, current - amount); break;
        ...
        if (user.id === userId) {
            window[currency] = newValue;
            updateUI();
        }

        const actionText = action === 'set' ? 'установлено' : action === 'add' ? 'начислено' : 'списано';
        resultP.style.color = '#27ae60';
        resultP.textContent = `✅ ${nickname}: ${actionText} ${amount} → баланс ${newValue.toFixed(2)} ${currency.toUpperCase()}`;
        document.getElementById('bal-amount').value = '';
    } catch(e) {
        resultP.textContent = 'Ошибка операции';
    }
};
    ...
    try {
        ...
        requests.forEach(r => {
            html += `<tr><td>${r.id}</td><td>${r.nickname||'-'}</td><td>${r.amount} SRUM</td>
                ...
                <td>${r.status==='pending' ? `<button class="admin-btn admin-btn-success" onclick="approveWithdrawal(${r.id})" style="padding:4px 8px; font-size:0.65rem;">✓</button><button class="admin-btn admin-btn-danger" onclick="rejectWithdrawal(${r.id})" style="padding:4px 8px; font-size:0.65rem;">✕</button>` : '-'}</td></tr>`;
        });
        html += '</tbody></table>';
...
async function loadTournaments(container) {
    container.innerHTML = `<button class="admin-btn admin-btn-success" onclick="createTournament()" style="width:100%; margin-bottom:15px;">🏆 Создать турнир</button><div id="tournaments-list"></div>`;
    try {
        ...
        tournaments.forEach(t => {
            html += `<div style="background:rgba(255,255,255,0.03); padding:10px; margin:5px 0; border-radius:8px; display:flex; justify-content:space-between;">
                ...
                <button class="admin-btn admin-btn-danger" onclick="deleteTournament(${t.id})" style="padding:4px 10px; font-size:0.65rem;">🗑️</button></div>`;
        });
...
function loadShopManager(container) {
    container.innerHTML = `
        ...
        <button class="admin-btn admin-btn-success" onclick="addShopItem()" style="width:100%;">✅ Добавить</button>
...
function renderShopItemsList() {
    ...
    shopItems.forEach(item => {
        html += `<div style="background:rgba(255,255,255,0.03); padding:8px; margin:4px 0; border-radius:6px; display:flex; justify-content:space-between; font-size:0.75rem;">
            ...
            <button class="admin-btn admin-btn-danger" onclick="removeShopItem(${item.id})" style="padding:2px 8px; font-size:0.6rem;">✕</button></div>`;
    });
...
function loadSettings(container) {
    container.innerHTML = `
        ...
        <div style="background:rgba(255,255,255,0.03); padding:12px; border-radius:10px; margin:10px 0;">
            ...
            <button class="admin-btn ${spartansEnabled?'admin-btn-danger':'admin-btn-success'}" onclick="toggleSpartans()" style="width:100%;">${spartansEnabled?'🛑 Выключить':'🟢 Включить'}</button>
            <button class="admin-btn admin-btn-info" onclick="resetSpartans()" style="width:100%; margin-top:5px;">🔄 Сбросить</button>
        </div>
        <div style="background:rgba(255,255,255,0.03); padding:12px; border-radius:10px; margin:10px 0;">
            ...
            <button class="admin-btn admin-btn-success" onclick="saveRates()" style="width:100%; margin-top:8px;">💾 Сохранить</button>
        </div>
