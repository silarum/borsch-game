...
function showBuySRUMModal() {
    ...
    if (!document.getElementById('buy-srum-style')) {
        ...
        style.textContent = `
            .buy-srum-btn {
                flex:1; padding:14px 8px; border:none; border-radius:12px; font-weight:bold; font-size:0.85rem;
...
function showMiningModal() {
    ...
    const rewardDisplay = document.getElementById('reward-text');
    slider.addEventListener('input', function () {
        const value = parseFloat(this.value);
        stakeDisplay.textContent = value.toFixed(1);
        penaltyDisplay.textContent = `Штраф: ${(value * getPenaltyPercent()).toFixed(2)} SRUM`;
        rewardDisplay.textContent = `Награда: ${(value * getRewardPercent()).toFixed(2)} USDT`;
    });
...
function startSearch(mode = 'mining') {
    ...
    fetch('https://hngfpdsnjgdpazmortix.supabase.co/functions/v1/matchmaking', {
        ...
        readyDiv.innerHTML = `<div style="text-align:center;">
            <p>Соперник: <b>${currentBot.name}</b></p>
...
async function endDuel(duelTimerInterval, duelSpawnInterval, duelBotInterval) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (miningCurrency === 'SRUM') { srum = data.srum; usdt = data.usdt; miningStage = data.mining_stage; }
            else { rum = data.rum; }
            if (win) {
                showCoinFountain(30);
                resultDiv.innerHTML = `<h2>⛏️ Блок добыт!</h2><p>+${(miningThreshold * getRewardPercent()).toFixed(2)} USDT</p><p>Этап ${miningStage}</p>
                    <button id="continue-mining">Продолжить</button><button id="pause-mining">⏸️ Пауза</button>`;
            } else {
                showPoopFountain(20);
                let phrase = cheerPhrases[Math.floor(Math.random() * cheerPhrases.length)];
                resultDiv.innerHTML = `<h2>💨 Блок упущен</h2><p>${phrase}</p><p>Потеря: ${(miningThreshold * getPenaltyPercent()).toFixed(2)} SRUM</p><p>Этап 1</p>
                    <button id="continue-mining">Продолжить</button><button id="pause-mining">⏸️ Пауза</button>`;
            }
        }
    ...
    if (currentBot.botIndex >= 0) {
        updateSpartanBot(currentBot.botIndex, !win, miningStage, (miningThreshold * getPenaltyPercent()), (miningThreshold * getRewardPercent()));
    }
    ...
    document.getElementById('continue-mining')?.addEventListener('click', ()=>{
        resultDiv.remove();
        if (miningCurrency === 'SRUM' && srum < miningThreshold) { showBuySRUMModal(); return; }
        if (miningCurrency === 'SRUM') srum -= miningThreshold;
        else rum -= miningThreshold;
        pendingMining = { currency: miningCurrency, threshold: miningThreshold, stage: miningStage };
        startSearch('mining');
...
function renderPausedSessions() {
    ...
    document.querySelectorAll('.resume-session').forEach(b => b.addEventListener('click', (e) => {
        ...
        miningCurrency = session.currency; miningThreshold = session.threshold; miningStage = session.stage;
...
arenaContent.addEventListener('click', function(e) {
    ...
    if (target.id === 'leave-band-btn') {
        ...
        if (bandData.fighters.length === 0) { bandData = null; alert('Банда распущена.'); }
