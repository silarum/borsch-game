...
-- These tables existed in the production project before migrations were
-- introduced, but do not exist on a clean preview branch. Guard every legacy
-- operation so the same migration remains valid in both environments.
do $$
begin
  if to_regclass('public.users') is not null then
    ...
    execute 'drop policy if exists users_service_all on public.users';
  ...
  if to_regclass('public.transactions') is not null then
    ...
    execute 'drop policy if exists "Allow all for anon" on public.transactions';
    execute 'revoke all privileges on table public.transactions from public, anon, authenticated';
    execute 'create index if not exists transactions_user_id_idx on public.transactions (user_id)';
  end if;
  ...
  if to_regclass('public.withdrawal_requests') is not null then
    ...
    execute 'drop policy if exists "Allow all for anon" on public.withdrawal_requests';
    execute 'drop policy if exists withdrawals_insert_own on public.withdrawal_requests';
    execute 'drop policy if exists withdrawals_select_own on public.withdrawal_requests';
    execute 'drop policy if exists withdrawals_service_all on public.withdrawal_requests';
    execute 'revoke all privileges on table public.withdrawal_requests from public, anon, authenticated';
    execute 'create index if not exists withdrawal_requests_user_id_idx on public.withdrawal_requests (user_id)';
  end if;

  if to_regclass('public.tournaments') is not null then
    execute 'alter table public.tournaments enable row level security';
    execute 'drop policy if exists "Allow all for anon" on public.tournaments';
    execute 'revoke all privileges on table public.tournaments from public, anon, authenticated';
    execute 'create index if not exists tournaments_created_by_idx on public.tournaments (created_by)';
  end if;
end;
$$;
